...
// ------ SENSOR CONSTANTS
#define sensor_RL 47  //The value of resistor RL is 47K
#define sensor_m -0.263 //Enter calculated slope 
#define sensor_b 0.42 //Enter calculated intercept
int sensor_Ro = 20; //Enter Ro value (resistance at fresh air)
bool do_calibrate_sensor = false;
String do_calibrate_sensor_string = "N";
bool begin_calibration_done = false;
int sensor_calibration_time = 24; // jam
unsigned long init_sensor_calibration;
...
unsigned long hold_duration = 5000; // hold increase button for 5s to setup AP wifi mode
...
unsigned long drain_time = 3000; // ms, draining time in case lower water sensor not used
unsigned long fill_time = 5000; // ms, draining time in case lower water sensor not used
bool use_drain_time = true;
bool use_fill_time = true;
...
float water_limit_upper = 600; // analog value
...
const char* PARAM_INPUT_KALIBRASI = "inputkalibrasi";
...
<!DOCTYPE HTML><html><head>
    ...
    <form action="/get">
        ...
        <input type="Radio" name="inputkalibrasi" Value="Y" Checked> Ya
        <input type="Radio" name="inputkalibrasi" Value="N"> Tidak
        <br>
...
void loop() {
    ...
    if (do_calibrate_sensor) {
        ...
        if ((millis() - init_sensor_calibration) < sensor_calibration_time) {
            lcd_clear_line(2);
            lcd.setCursor(0, 2);
            lcd.print("RESISTANSI RO:   k");
            lcd.setCursor(15, 2);
            lcd.print(sensor_Ro);
            
            Serial.print("RESISTANCE RO: ");
            Serial.println(sensor_Ro);
    ...
    // Determine main fsm state and cycle main fsm
    determine_state();
    FSM();
...
void fsm_chamber() {
    switch (chamber_state) {
        ...
        case FILL:
            ...
            if (use_fill_time) {
                if (chamber_time - init_chamber_time > fill_time) {
...
void begin_sensor_calibration() {
    ...
    lcd.print("TUNGGU");
    lcd.setCursor(7, 1);
    lcd.print(sensor_calibration_time);
    lcd.setCursor(10, 1);
    lcd.print("JAM");
    
    init_sensor_calibration = millis();
    sensor_calibration_time = sensor_calibration_time * 3600 * 1000; // convert to ms 
    begin_calibration_done = true;
}
...
void read_sensor() {
    ...
    float sensor_V; // Voltage drop across sensor
    float sensor_Rs; // Resistance at gas concentration 
    float sensor_ratio; // Rs/Ro
    float ppm; // Gas concentration in ppm
    ...
    sensor_V = analogRead(AMMONIA_SENSOR)*(3.3/4095.0); // Measure the voltage drop and convert to 0-5V
    
    sensor_Rs = ((3.3*sensor_RL)/sensor_V)-sensor_RL; // Use formula to get Rs value
    
    sensor_ratio = sensor_Rs/sensor_Ro;
    
    ppm = pow(10, ((log10(sensor_ratio) - sensor_b) / sensor_m)); //use formula to calculate ppm
...
void calibrate_reading() {
    ...
    // calibrated_measurement = (m * calibrated_measurement) + b;
}
...
void setup_webserver() {
    ...
    server.on("/get", HTTP_GET, [] (AsyncWebServerRequest *request) {
        ...
        if ((request->hasParam(PARAM_INPUT_SSID)) && (request->hasParam(PARAM_INPUT_PASS)) 
         ...
         && (request->hasParam(PARAM_INPUT_KALIBRASI))) {
            ...
            inputKALIBRASI = request->getParam(PARAM_INPUT_KALIBRASI)->value();
            
            if ((inputSSID == "") || (inputPASS == "") || (inputCHATID == "") || (inputREPORTINTERVAL == "") || (inputKALIBRASI == "")) {
                Serial.println("Masukkan semua informasi yang diperlukan.");
                request->send(200, "text/html", "<p style=\"font-size:30px;\">Masukkan semua informasi yang diperlukan.</p><br><a href=\"/\">Return to home page.</a>");
            }
            else {
                ...
                if (inputKALIBRASI == "Y") {
                    do_calibrate_sensor = true;
                    begin_calibration_done = false;
                }
                else { // "N"
                    do_calibrate_sensor = false;
                }
...
void check_telegram_bot() {
    if (millis() > last_bot_report + bot_report_interval)  {
        if (do_calibrate_sensor) {
            // String msg = "ALAT SEDANG DIKALIBRASI\n";
            // msg += "Mohon tunggu ";
            // msg += String(sensor_calibration_time/(3600 * 1000));
            // msg += " jam.";
